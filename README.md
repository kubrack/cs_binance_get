...
A [re-frame](https://github.com/day8/re-frame) application designed to get prices from Binance then make graph on it.
...
* [`src/cljs/cs_binance_get/`](src/cljs/cs_binance_get/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/cs_binance_get/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `cs_binance_get` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cs_binance_get/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cs-binance-get.example
  (:require [cs-binance-get.config :as config])
